...
class MapManager {
            ...
            closure()
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                              self.showAlert(title: "Your Location is not Availeble",
                                             message: "To give permission Go to: Setting - MyPlaces - Location Services and turn On")
                          }
           }
       }
    ...
    func getDirections(for mapView: MKMapView, previousLocation: (CLLocation) -> ()) {
        ...
        directions.calculate { (response, error) in
            ...
            for route in response.routes {
                ...
                print("Растоянние до места: \(distance) км.")
    ...
    //Меняем отображаемую зону области карты в соответствии с пермещением пользователя
